  ...
  fetch(`https://tmi.twitch.tv/group/user/${channel}/chatters`)
    ...
    .then((resp) => {
      var usersWithoutData = [],
        dataToResponse = [],
        responseSize =
          resp.chatter_count >= chattersPerRequest
            ? chattersPerRequest
            : resp.chatter_count;
      ...
      process.nextTick(() => {
        updateUsersData(usersWithoutData).then(() => {
          while (dataToResponse.length < responseSize) {
            let userName = usersWithoutData.shift();
            dataToResponse.push(
              data.usersData.get(userName) || { login: userName }
            );
          }
...
ipcMain.on("chatters-filter", (event, channel, query) => {
  ...
  if (resp) {
    ...
    process.nextTick(() => {
      ...
      while (
        dataToSend.length < chattersPerRequest &&
        dataToSend.length < filtered.length
      ) {
