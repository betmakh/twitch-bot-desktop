...
class ChatComponent extends React.Component {
  ...
  render() {
    ...
    return (
      <div
        className={classes.bottomContainerWrapper}
        ...
        <AppBar position="static" color="primary" className={classes.header}>
          <Toolbar>
            <IconButton onClick={this.toggleAutoplay.bind(this)}>
              {!commentsAutoplay ? (
                <VolumeOffIcon title="Unmute" />
              ) : (
                <VolumeUpIcon title="Mute" />
              )}
            </IconButton>
            <Typography
              variant="title"
            ...
            {messages.length ? (
              messages.map((msg, index) => (
                <Grid
                  ...
                  key={msg.id}
                  ref={
                    index === messages.length - 1 ? this.scrollToBottom : null
                  }
                >
                  <Paper
                    className={`${classes.card} ${
                      msg.isAction ? classes.actionMessage : null
                    }`}
                    square={msg.isAction}
                  >
